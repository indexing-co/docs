...
We'd first [add a filter](/api-docs/endpoint/filters/add) for his address, `0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045`.
...
NOTE: the above transformation does _not_ have its own filter step. Instead, we can specify during [testing](/api-docs/endpoint/transformations/test) and in the [pipeline](/api-docs/endpoint/pipelines/create) to filter by `from`, `to`, or both.
