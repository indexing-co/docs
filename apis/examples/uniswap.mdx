...
If we save the above code as `swaps.js`, then we can leverage it to test against any EVM network + block using the [test](/apis/transformations/test) endpoint. Here's how we could do it as a CURL request:
...
Once we're happy with the results, we can commit the transformation using the [create](/apis/transformations/create) endponit like this:
...
You can [create](/apis/pipelines/create) the pipeline with the following payload:
